...
if __name__=='__main__':
    ...
    try:
        ...
        #si reel
        if lidar_topic=="/scan":
            rospy.Subscriber(lidar_topic, LaserScan, lidar_preprocess_callback,c,queue_size=1)
        
        #si simu
        elif lidar_topic=="/LidarScan":
            rospy.Subscriber(lidar_topic, Float32MultiArray, lidar_preprocess_callback,c,queue_size=1)
        ...
        front_pub=rospy.Publisher(front_topic,Float32MultiArray,queue_size=1)
        
        #publisher side_data
        side_topic=rospy.get_param("~side_topic",default="/side_data") 
        side_pub=rospy.Publisher(side_topic,Float32MultiArray,queue_size=1)
